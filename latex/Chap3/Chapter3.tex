...
\begin{enumerate}
    \item \textbf{Initial Validation}: Ensuring required columns (e.g., \texttt{LogTime}, \texttt{LogFloatValue}) exist.
    \item \textbf{Type Conversion}: Converting timestamps to datetime objects and sensor values to appropriate numeric types.
    \item \textbf{Missing Value Handling}: Removing or imputing rows with critical missing values.
    \item \textbf{Duplicate Removal}: Eliminating redundant records based on unique sensor/timestamp combinations.
    \item \textbf{Domain Validation}: Checking if values fall within realistic physical ranges (e.g., humidity between 0-100\%).
    \item \textbf{Context-Aware IQR Outlier Flagging}: Unlike global outlier detection, this stage computes Interquartile Range (IQR) fences for each specific \texttt{LogType} group. This ensures that a temperature reading is compared only against other temperature readings, preventing false positives caused by differing scales across sensor types.
    \item \textbf{Feature Engineering}:
    \begin{itemize}
        \item \textbf{Time Delay Calculation}: Computing the difference between \texttt{ServerTime} and \texttt{LogTime}.
        \item \textbf{Temporal Features}: Extracting the hour of the day to capture diurnal patterns.
    \end{itemize}
    \item \textbf{Normalization}: Scaling features (using StandardScaler) to ensure the models are not biased toward columns with larger numerical ranges.
    \item \textbf{Categorical Encoding}: One-hot encoding categorical variables like \texttt{LogTypeID} to make them usable by the algorithms.
\end{enumerate}
